...
def generate_csv_report(shipments, filename):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for shipment in shipments:
            to_addr = shipment.to_address
            from_addr = shipment.from_address
            ...
            writer.writerow({
                ...
                'to_name': to_addr.name if to_addr else 'N/A',
                'to_street1': to_addr.street1 if to_addr else 'N/A',
                'to_city': to_addr.city if to_addr else 'N/A',
                'to_state': to_addr.state if to_addr else 'N/A',
                'to_zip': to_addr.zip if to_addr else 'N/A',
                'to_country': to_addr.country if to_addr else 'N/A',
                'from_name': from_addr.name if from_addr else 'N/A',
                'from_company': from_addr.company if from_addr else 'N/A',
                'from_street1': from_addr.street1 if from_addr else 'N/A',
                'from_city': from_addr.city if from_addr else 'N/A',
                'from_state': from_addr.state if from_addr else 'N/A',
                'from_zip': from_addr.zip if from_addr else 'N/A',
                'from_country': from_addr.country if from_addr else 'N/A',
                'parcel_length': parcel.length if parcel else 'N/A',
                'parcel_width': parcel.width if parcel else 'N/A',
                'parcel_height': parcel.height if parcel else 'N/A',
                'parcel_weight': parcel.weight if parcel else 'N/A',
            })
